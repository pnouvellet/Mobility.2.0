...
pdf(file = '../../../../Dropbox (SPH Imperial College)/Mobility.2.0.Save/Saved_file/TableModels_comparision/figure4.pdf',
    width = 8, height=10)
...
for (i in 1:length(idx)){
  ...
  if(AG$Tc[idx[i]]){
    yl[i] = paste0(gsub('_',' ',AG$country[idx[i]]),' (',
                   round(100*AG$Rsq1[idx[i]],digits = 0),' ; ',
                   round(100*AG$Rsq2[idx[i]],digits = 0),')')
    
    if(AG$beta1[i]==1){
      x <- c(AG$th1[idx[i]], AG$th1low[idx[i]],AG$th1up[idx[i]] )
      x <- bound(x)
      lines( x[1], i+.1,type = 'p', pch=16,col = 'darkslategray4')
      lines(x[2:3], c(i,i)+.1,lwd=2,col = 'darkslategray4')
    }
    if(AG$beta2[i]==1){
      x <- c(AG$th2[idx[i]], AG$th2low[idx[i]],AG$th2up[idx[i]] )
      x <- bound(x)
      lines( x[1], i-.1,type = 'p', pch=16,col = 'darkorange1')
      lines(x[2:3], c(i,i)-.1,lwd=2,col = 'darkorange1')
    }
    
    
  }else{
    yl[i] = paste0(gsub('_',' ',AG$country[idx[i]]),' (',
                   round(100*AG$Rsq[idx[i]],digits = 0),')')
    
    if(AG$beta1[i]==1){
      x <- c(AG$th1[idx[i]] , AG$th1low[idx[i]],AG$th1up[idx[i]])
      x <- bound(x)
      lines( x[1], i,type = 'p', pch=16,col = 'darkslategray4')
      lines(x[2:3], c(i,i),lwd=2,col = 'darkslategray4')
    }
...
mtext(side = 3, text = 'Country ( R-sq(s) )',adj = -.4,padj = 2.5,cex=1, font =3)
```
...
pdf(file = '../../../../Dropbox (SPH Imperial College)/Mobility.2.0.Save/Saved_file/TableModels_comparision/figure3.pdf',
    width = 7, height=4)
