niceplot <- function(i,res_summary, res_base,inputs){
  ...
  #   lines(inputs$D$dates[round(res_m2$tchange[1,i])],5.5,type = 'p',pch=16,col='black')
  #   lines(inputs$D$dates[round(res_m2$tchange[2:3,i])],rep(5.5,2),lwd=1.5)
  # # }
  # if (Change ==0){
 ...
 ###################################
  ...
  # if (Change == 1){
    lines(res_summary2$results_full_Rt_assumed_mob$median_R$mobility,
